  ...
  return (
    <div style={{ position: "relative", width: 140, height: 140, flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 32, fontWeight: 800, color: cfg.color, fontFamily: "'Bebas Neue', sans-serif", letterSpacing: 1 }}>{score}</span>
        <span style={{ fontSize: 10, color: "#666", textTransform: "uppercase", letterSpacing: 2 }}>score</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, color: "#ccc", fontVariantNumeric: "tabular-nums" }}>{value}{unit}</span>
      </div>
...
function NutriscoreBadge({ grade }) {
  ...
  const colors = { a: "#1a7a4a", b: "#56a832", c: "#d4c400", d: "#e67800", e: "#c41800" };
...
function ResultView({ result, onReset }) {
  ...
  const chartData = [
    { name: "Calories", value: nutrition.calories || 0, max: 600, color: "#e879f9" },
    { name: "Protein",  value: nutrition.protein  || 0, max: 50,  color: "#60a5fa" },
    ...
    { name: "Sodium",   value: nutrition.sodium   || 0, max: 800, color: "#94a3b8", unit: "mg" },
  ];
  ...
  return (
    <div style={{ animation: "fadeUp 0.5s ease" }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 180 }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
            ...
            {result.source && <Tag>{SOURCE_LABELS[result.source] || result.source}</Tag>}
          </div>
          <h2 style={{
            margin: "0 0 4px", fontSize: 22, fontWeight: 800,
            color: "#f0f0f0", fontFamily: "'Bebas Neue', sans-serif",
            letterSpacing: 1.5, textTransform: "uppercase",
          }}>
            {result.foodName}
          </h2>
          {result.brand && (
            <p style={{ margin: "0 0 6px", fontSize: 12, color: "#666" }}>by {result.brand}</p>
          )}
          <p style={{ margin: 0, fontSize: 13, color: "#999", lineHeight: 1.5 }}>{result.reason}</p>
        </div>
      </div>
      ...
      {result.warning && (
        <div style={{
          background: "#1a1500", border: "1px solid #fbbf2440",
      ...
      {result.alternativeDetections?.length > 0 && (
        <div style={{
          ...
          <p style={{ margin: "0 0 8px", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, color: "#555" }}>
            Alternative Detections
          </p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {result.alternativeDetections.map((d, i) => (
              <span key={i} style={{
                ...
                background: "#1a1a2e", padding: "4px 10px", borderRadius: 20,
              }}>
      ...
      <div style={{
        ...
        <p style={{ margin: "0 0 14px", fontSize: 11, textTransform: "uppercase", letterSpacing: 2, color: "#555" }}>
          Nutrition per 100g
        </p>
        {chartData.map(d => (
          <NutriBar key={d.name} label={d.name} value={d.value}
            unit={d.unit || "g"} max={d.max} color={d.color} />
        ))}
      </div>
      ...
      {result.ingredients && (
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: 12, color: "#777", lineHeight: 1.7 }}>{result.ingredients}</p>
        </div>
      )}

      {/* Detection confidence */}
      {result.detectionConfidence != null && (
        <p style={{ fontSize: 11, color: "#444", textAlign: "right", margin: "0 0 16px" }}>
      ...
      <button onClick={onReset} style={{
        ...
        borderRadius: 8, color: "#666", fontSize: 13,
        letterSpacing: 1.5, textTransform: "uppercase",
...
function BarcodeTab({ goal, onResult, loading, setLoading }) {
  ...
  return (
    <div>
      <p style={{ margin: "0 0 12px", fontSize: 12, color: "#555", letterSpacing: 1 }}>
      ...
      <input
        ...
        style={{
          ...
          color: "#eee", fontSize: 15, fontFamily: "monospace", letterSpacing: 2,
          outline: "none",
        }}
      />
      <button onClick={submit} disabled={loading || !barcode.trim()} style={{
        width: "100%", marginTop: 12, padding: "13px 0",
        background: loading || !barcode.trim()
          ? "#1a1a2e"
          : "linear-gradient(135deg, #06b6d4, #3b82f6)",
        border: "none", borderRadius: 8, color: loading || !barcode.trim() ? "#555" : "#fff",
        fontSize: 13, fontWeight: 700, letterSpacing: 2,
        textTransform: "uppercase", cursor: loading || !barcode.trim() ? "default" : "pointer",
        transition: "all 0.2s",
      }}>
  ...
  return (
    <div>
      ...
      <button onClick={submit} disabled={loading || !query.trim()} style={{
        ...
        border: "none", borderRadius: 8, color: loading || !query.trim() ? "#555" : "#fff",
        fontSize: 13, fontWeight: 700, letterSpacing: 2,
        textTransform: "uppercase", cursor: loading || !query.trim() ? "default" : "pointer",
        transition: "all 0.2s",
      }}>
...
const TABS = [
  ...
  { id: "barcode", label: "⬡ Barcode",  color: "#06b6d4" },
      ...
      <div style={{ minHeight: "91vh", padding: "24px 16px 48px" }}>
        <div style={{ width: "80%", margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28, animation: "fadeUp 0.4s ease" }}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#333", letterSpacing: 3, textTransform: "uppercase" }}>
              Know what you eat
            </p>
          </div>
          ...
          {!result ? (
            <div style={{ animation: "fadeUp 0.45s ease" }}>
              ...
              <div style={{ marginBottom: 15 }}>
                <p style={{ margin: "0 0 8px", fontSize: 20, color: "#444", letterSpacing: 2, textTransform: "uppercase" }}>
                  Your Goal
                </p>
                <div style={{ display: "flex", gap: 8 }}>
                  {GOALS.map(g => (
                    <button key={g.value} onClick={() => setGoal(g.value)} style={{
                      ...
                      background: goal === g.value ? "#1a1a2e" : "transparent",
                      border: `1px solid ${goal === g.value ? "#333" : "#1a1a2e"}`,
                      borderRadius: 6, color: goal === g.value ? "#aaa" : "#444",
              ...
              <div style={{
                ...
                {tab === "image"   && <ImageTab   goal={goal} onResult={setResult} loading={loading} setLoading={setLoading} />}
                {tab === "barcode" && <BarcodeTab goal={goal} onResult={setResult} loading={loading} setLoading={setLoading} />}
                {tab === "text"    && <TextTab    goal={goal} onResult={setResult} loading={loading} setLoading={setLoading} />}
              </div>
              ...
              {loading && (
                <div style={{
                  ...
                  <p style={{ margin: 0, fontSize: 12, color: "#444", letterSpacing: 1.5, textTransform: "uppercase" }}>
